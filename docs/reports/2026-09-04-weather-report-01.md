...
- **There is exactly one observer, and it is me.** This is a one-person
  project, and that matters for reading everything below. A public drift alert
...
- **The live board's baseline was re-established on 2026-08-04.** There
  ...
  the postmortem. That is a synthetic replay, not a live catch, and I do
  not describe it as one.

Last week I shipped a small read-side change: `/v1/weather` now
...
without its base is a defect; I had been doing it.
...
defect. I did not anticipate this when I scoped the change.
...
byte-identical bounds, because not one new row arrived. The leg I was
...
I reconciled the board against the scheduler for the window
...
endpoint. I do not yet know which, and that is its own finding -- the
run log records the failed prompt ids and nothing else. No status code,
no response body, no retry spend. **I built a drift detector whose own
collection failures are not diagnosable from its own logs.** That gets
fixed before the collection logic does.

One more thing worth stating because it invalidates arithmetic I had
...
This is the finding I would most want a reader to take away, and it is
uncomfortable.
...
has no new data. **Read those as "nothing to report from a sample I
...
tell me the collection design is wrong, the code and the board are open.
